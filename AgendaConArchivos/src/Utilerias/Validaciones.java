...
public class Validaciones {
    ...
    public static boolean verificarExistenciaNombre(String n, Hashtable<String, Contacto> agenda) {
        n = n.toUpperCase();
        Enumeration<String> enumK = agenda.keys();

        while (enumK.hasMoreElements()) {
            String llave = enumK.nextElement().toUpperCase();
            if (agenda.containsKey(llave)) {
                if (agenda.get(llave).getNombre().equalsIgnoreCase(n)) {
                    return true;
                }
            }
        }
    ...
    public static boolean validaExistenciaArchivo(char l) {
        switch (l) {
            case 'S':
                return true;
            case 's':
